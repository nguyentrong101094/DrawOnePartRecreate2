...
public class GestureRecognizerInGame : MonoBehaviour
{
    public event System.EventHandler<Gesture> onUserDrawGesture;
    public event System.EventHandler<Gesture> onGestureRecognized;
    ...
    /// </summary>
    GestureLibrary gl;
    public GestureLibrary LoadedGestureLibrary { get => gl; }
    ...
    {
        libraryToLoad = data.name.ToString();
        gl = new GestureLibrary(libraryToLoad, forceCopy);
        DrawBoundMaker.SetupGestureBound(data, gestureLimitRectBounds);
        ...
        gestureLimitRectBounds.transform.position = gestureLimitRectBounds.transform.position + new Vector3(0f, Const.ADDITION_PLAY_FIELD_Y_WORLD);
        ...
        canvasCamera = SS.View.Manager.Object.UICamera;
    }
        ...
        if (isEnabled)
        {
            ...
            if (Input.GetMouseButton(0))
            {
                ...
                switch (gestureLimitType)
                {
                    ...
                    case GestureLimitType.RectBoundsIgnore:
                        if (RectTransformUtility.RectangleContainsScreenPoint(gestureLimitRectBounds, virtualKeyPosition, canvasCamera))
                        {
                            RegisterPoint();
                        }
                        break;
            ...
            if (Input.GetMouseButtonUp(0))
            {
                ...
                if (points.Count > minimumPointsToRecognize)
                {
                    ...
                    result = gesture.Recognize(gl, out Gesture recognizedGesture, useProtractor);

                    if (OnRecognition != null)
                    {
                        OnRecognition(result);
                    }
                    onUserDrawGesture?.Invoke(this, gesture);
                    onGestureRecognized?.Invoke(this, recognizedGesture);
                }
