...
namespace PaintIn3D
{
	public partial class P3dPainter
	{
		...
		private void HandleDraw()
		{
			...
			if (ClearPath(Event.current.mousePosition) == true && currentPaintMaterial != null && currentLayer != null && currentPaintBrush != null)
			{
				...
				{
					if (Event.current.type == EventType.MouseDown || Event.current.type == EventType.MouseDrag || Event.current.type == EventType.MouseMove || Event.current.type == EventType.Repaint)
					{
						DoPaint(painting == false, ray, hit);
					}
				}
			}
		}
