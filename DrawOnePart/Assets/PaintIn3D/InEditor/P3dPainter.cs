...
namespace PaintIn3D
{
	public partial class P3dPainter : P3dEditorWindow
	{
		...
		private P3dPaintBrush currentPaintBrush;
		...
		[SerializeField] private Vector2 brushesScrollPosition;
		[SerializeField] private Vector2 paintScrollPosition;
		...
		private void DrawViewport()
		{
			...
			if (fullRect.width > 0 && fullRect.height > 0)
			{
				...
				lights[0].transform.rotation = Quaternion.Euler(settings.Light0.Pitch, settings.Light0.Yaw, 0.0f);
				lights[0].intensity = settings.Light0.Intensity;
				lights[0].color     = settings.Light0.Color;

				lights[1].transform.rotation = Quaternion.Euler(settings.Light1.Pitch, settings.Light1.Yaw, 0.0f);
				...
				foreach (var obj in scene.Objs)
				{
					if (obj.Mesh != null)
					{
						for (var i = 0; i < obj.MatIds.Count; i++)
						{
							...
							var subMesh = Mathf.Min(i, obj.Mesh.subMeshCount);
