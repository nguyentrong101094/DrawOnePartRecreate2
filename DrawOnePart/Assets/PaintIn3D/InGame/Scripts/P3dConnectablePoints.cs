...
namespace PaintIn3D
{
	...
	public abstract class P3dConnectablePoints : MonoBehaviour
	{
		...
		protected void SubmitPoint(bool preview, int priority, Collider collider, Vector3 worldPosition, Quaternion worldRotation, float pressure, object owner)
		{
			if (owner != null)
			{
				...
				if (TryGetLink(owner, ref link) == true)
				{
					if (link.Preview == preview)
					{
						if (hitSpacing > 0.0f)
						{
							var currentPosition = link.WorldPosition;
							...
							{
								currentPosition = Vector3.MoveTowards(currentPosition, worldPosition, hitSpacing);

								if (connectHits == true)
								{
									hitCache.InvokeLines(gameObject, preview, priority, link.WorldPosition, currentPosition, worldRotation, pressure);
								}
								else
								{
									hitCache.InvokePoints(gameObject, preview, priority, collider, currentPosition, worldRotation, pressure);
								}

								link.WorldPosition = currentPosition;
							}
						...
						{
							hitCache.InvokeLines(gameObject, preview, priority, link.WorldPosition, worldPosition, worldRotation, pressure);
						}
						else
						{
							hitCache.InvokePoints(gameObject, preview, priority, collider, worldPosition, worldRotation, pressure);
						}
					}
					else
					{
