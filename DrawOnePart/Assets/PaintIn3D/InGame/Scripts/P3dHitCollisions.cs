...
namespace PaintIn3D
{
	...
	public class P3dHitCollisions : MonoBehaviour
	{
		...
		/// <summary>The relative speed required for a hit to occur with a pressure of 0.</summary>
		public float SpeedMin { set { speedMin = value; } get { return speedMin; } } [UnityEngine.Serialization.FormerlySerializedAs("threshold")] [SerializeField] private float speedMin = 1.0f;

		/// <summary>The relative speed required for a hit to occur with a pressure of 1.</summary>
		public float SpeedPressure { set { speedPressure = value; } get { return speedPressure; } } [SerializeField] private float speedPressure = 100.0f;
		...
		private void CheckCollision(Collision collision)
		{
			...
			if (speed >= speedMin)
			{
				...
				for (var i = contacts.Length - 1; i >= 0; i--)
				{
					...
					hitCache.InvokePoints(gameObject, false, priority, contact.otherCollider, finalPosition, finalRotation, 1.0f);
					...
					if (raycastDistance > 0.0f)
					{
						...
						{
							hitCache.InvokeRaycast(gameObject, false, priority, hit, pressure);
						}
					}
...
namespace PaintIn3D
{
	...
	public class P3dHitCollisions_Editor : P3dEditor<P3dHitCollisions>
	{
		...
		{
			Draw("speedMin", "The relative speed required for a hit to occur with a pressure of 0.");
			Draw("speedPressure", "The relative speed required for a hit to occur with a pressure of 1.");
			...
			Target.HitCache.Inspector(Target.gameObject, false, true, false, false);
		}
	}
}
#endif