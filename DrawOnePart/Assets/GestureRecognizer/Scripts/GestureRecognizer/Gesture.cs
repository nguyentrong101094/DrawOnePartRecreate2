...
namespace GestureRecognizer
{
    ...
    public partial class Gesture
    {
        ...
        /// </summary>
        public static float SQUARE_SIZE { get { return 250f; } }
        ...
        public Gesture(List<Vector2> points, string name = "")
        {
            ...
            this.Points = this.ScaleTo(SQUARE_SIZE);
            this.Points = this.TranslateTo(this.ORIGIN);
            this.Vector = this.Vectorize();
        }

        public Gesture(List<Vector2> points, bool useDrawDotWorldPos)
        {
            this.Points = points;
            this.Points = this.Resample(NUMBER_OF_POINTS);
        }
        ...
        /// <summary>
        /// Scale only gesture that is smaller than predefined bounding box 
        /// </summary>
        /// <param name="size">Size of the bounding box to scale to</param>
        /// <returns>List of points which now fits in the predefined bounding box.</returns>

        public List<Vector2> ScaleOnlySmaller(float size)
        {
            Rect boundingBox = Gesture.GetBoundingBox(this.Points);
            if (boundingBox.width < size || boundingBox.height < size)
            {
                Debug.Log("Small gesture will be scaled");
                List<Vector2> scaledPoints = new List<Vector2>();
                float scaleValue;
                ...
                else scaleValue = size / boundingBox.height;
        ...
        public static float GetDistanceBetweenPaths(List<Vector2> points1, List<Vector2> points2)
        {
            ...
            for (int i = 0; i < points1.Count; i++)
            {
                if (i >= points2.Count)
                {
                    Debug.LogError("invalid points 2");
                    distance = 999999f;
                    break;
                }
